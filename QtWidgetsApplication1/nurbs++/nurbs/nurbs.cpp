...
void NurbsCurve<T,N>::drawAaImg(Image_Color& Img, const Color& color, int precision, int alpha){
  ...
  profile.makeCircle(Point_nD<T,3>(0,0,0),Point_nD<T,3>(1,0,0),Point_nD<T,3>(0,0,1),1.0,0,MY_M_PI) ;
...
void NurbsCurve<T,N>::makeCircle(const Point_nD<T,N>& O, const Point_nD<T,N>& X, const Point_nD<T,N>& Y, T r, double as, double ae){
  ...
  while(ae<as)
    ae += 2*MY_M_PI ;
  theta = ae-as ;
  if(theta<=MY_M_PI/2.0)	
    narcs = 1 ;
  else{
    if(theta<=MY_M_PI)
      narcs = 2 ;
    else{
      if(theta<=1.5*MY_M_PI)
	narcs = 3 ;
      else
	narcs = 4 ;
    }
...
int NurbsCurve<T,N>::writeVRML(const char* filename,T radius,int K, const Color& color,int Nu,int Nv, T u_s, T u_e) const{
  ...
  C.makeCircle(Point_nD<T,N>(0,0,0),Point_nD<T,N>(1,0,0),Point_nD<T,N>(0,0,1),radius,0,2*MY_M_PI);
  S.sweep(*this,C,K) ;
