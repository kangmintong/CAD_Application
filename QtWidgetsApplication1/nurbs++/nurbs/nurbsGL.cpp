...
/*!
  ...
  \date 20 September 1997
*/
const char* ObjectGL::typeName() const {
  switch(type){
...
/*!
  ...
  \date 23 September 1997
*/
NurbsGL* readNurbsObject(const char* infilename) {
  NurbsGL *temp ;
  ...
  //}
  //else{
  char *filename = (char*)malloc(strlen(infilename)*sizeof(char));
...
void NurbsCurveGL::gluNurbs() const{
   if(nurbsRenderer){
    ...
    //glColor() ;
	int knotSize = U.n();
	float *pKnot = new float[knotSize];	
	for(int i = 0; i < knotSize; i++){
		pKnot[i] = (U.memory())[i];
	}
	auto ctrlP = new float[P.size()][4];
	for(int i = 0; i < P.size(); i++){
		for(int j = 0; j < 4; j++){
			ctrlP[i][j] = P[i].data[j];
		}
	}
	gluNurbsCurve(nurbsRenderer, knotSize, pKnot, 4, &ctrlP[0][0], deg_+1, t);
	...
	glBegin(GL_POINTS);
	for(auto& kp : P){
		glVertex3f(kp.data[0], kp.data[1], kp.data[2]);
	}
	glEnd();
