...
public:
  ...
  MatrixRT<T>& rotateDeg(T ax, T ay, T az) { return rotate(T(ax*MY_M_PI/180.0),T(ay*MY_M_PI/180.0),T(az*MY_M_PI/180.0)) ; } 

  //! A rotation in the X,Y and Z order with the angles specified in degree
  MatrixRT<T>& rotateDegXYZ(T ax, T ay, T az) { return rotateXYZ(T(ax*MY_M_PI/180.0),T(ay*MY_M_PI/180.0),T(az*MY_M_PI/180.0)) ; } 
